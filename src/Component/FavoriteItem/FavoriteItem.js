...
import { addFavorite } from "../../redux/actions/actionCreater";
...
export default function FavoriteItem(props) {
  ...
  useEffect(() => {
    const checkIsFavorite = !!favoriteHotels.find(
      ({ hotelId }) => props.hotelId === hotelId
    );
    setIsFavorite(checkIsFavorite);
  }, [favoriteHotels, props.hotelId]);

  return (
    <li key={props.hotelId} className="HotelItem_list_item FavoriteItem_list">
      <div className="HotelItem_list_item_one">
        ...
        <div className="HotelItem_block_two">
          <div className="HotelItem_heading">
            <h2 className="HotelItem_list_item_head">{props.hotelName}</h2>
          </div>
          <div className="HotelItem_block_date">
            <p className="HotelItem_list_item_date">{props.bookingDate}</p>
            <p className="HotelItem_list_item_date_padding">-</p>
            <p className="HotelItem_list_item_day">{props.day} день</p>
          </div>
          <div>Рейтинг отеля {props.stars} из 5</div>
        </div>
      </div>
      <div className="HotelItem_block_three">
        <button className="HotelItem_svg" onClick={handleAdd}>
          <svg
            width="23"
            height="20"
            viewBox="0 0 23 20"
        ...
        <p className="HotelItem_price">
          Price:
          <strong className="place_price">
            {props.priceHotel * props.day} Р
          </strong>
        </p>
      </div>
    </li>
